...
#define LOOPBACK_QUEUE_LIMIT 32
...
/* デバイスに送信されたデータをいれるキューのエントリ */
struct loopback_queue_entry
{
    uint16_t type;
    size_t len;
    uint8_t data[]; /* flexible array memeber */
};
...
loopback_transmit(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst)
{
    ...
    mutex_unlock(&PRIV(dev)->mutex);
    debugf("queue pushed (num:%u), dev=%s, len=%zd", num, dev->name, type, len);
    debugdump(data, len);
...
loopback_isr(unsigned int irq, void *id)
{
    ...
    while (1)
    {
        ...
        debugf("quque popped, (num=%u), dev=%s, type=0x%04x, len=%zd", PRIV(dev)->queue.num, dev->name, entry->type, entry->len);
        debugdump(entry->data, entry->len);
        // net_input_handlerに受信データ本体と付随する情報を渡す
