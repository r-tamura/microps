...
int ether_addr_pton(const char *p, uint8_t *n)
{
    ...
    {
        val = strtol(p, &ep, 16);
        if (ep == p || val < 0 || val > 255 || (index < ETHER_ADDR_LEN - 1 && *ep != ':'))
        {
            break;
        }
        n[index] = (uint8_t)val;
...
int ether_transmit_helper(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len,
                          const void *dst, ether_transmit_func_t callback)
{
    ...
    hdr = (struct ether_hdr *)frame;
    /*
     * Ethernetフレームの構造(ChatGPTに描いてもらったものを少し修正)
     * +---------+----------------+----------------+----------------+----------------+----------------+
     * | 7 Octets |   6 Octets    |   6 Octets     |   2 Octets     | 46-1500 Octets |   4 Octets     |
     * | Preamble | Destination   |    Source      |   Ethertype    |    Payload     |      FCS       |
     * |          |   MAC Addr    |   MAC Addr     |  (2 Octets)    |  (46-1500 Octets) |  (4 Octets) |
     * +---------+----------------+----------------+----------------+----------------+----------------+
     */
    /* Ethernetフレームの生成 */
