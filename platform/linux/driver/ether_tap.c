#define _GNU_SOURCE /* for FSETSIG */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
ether_tap_open(struct net_device *dev)
{
    ...
    {
        errorf("fntl [F_SETSIG]: %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
...
int ether_tap_transmit(struct net_device *dev, uint16_t type, const uint8_t *buf,
...
ether_tap_isr(unsigned int irq, void *id)
{
    ...
    while (1)
    {
        ...
        // タイムアウト時間が0だとノンブロッキングな挙動になる。
        // > timeout を 0 に指定した場合、I/O 可能なファイルディスクリプターがない場合であっても、 poll() はすぐに返る
        // [Man page of POLL](https://linuxjm.osdn.jp/html/LDP_man-pages/man2/poll.2.html)
        ret = poll(&pfd, 1, 0);
        if (ret == -1)
        {
            // EINTRの場合は再試行（シグナルに割り込まれたという回復可能なエラー）
            if (errno == EINTR)
            {
                continue;
            }
...
ether_tap_init(const char *name, const char *addr)
{
    ...
    infof("etherenet device initialized, dev=%s", dev->name);
    return dev;
}